...
with open("results/rois.json", "r") as f:
    data = json.load(f)
...
for roi in data:
    ...
    # Numéro du ROI en haut à gauche du rectangle
    cv2.putText(image, f"ROI {roi['id']}",
                (x, y - 8),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5, (0, 255, 0), 1)

    print(f"ROI {roi['id']} → x={x}, y={y}, w={w}, h={h}")

cv2.imshow("Surface — ROIs", image)
cv2.waitKey(0)
cv2.destroyAllWindows()